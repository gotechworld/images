...
COPY tags/php/docker-php-source /usr/local/bin/
...
# Install PHP utils
#####################################
COPY tags/php/docker-php-ext-* tags/php/docker-php-entrypoint /usr/local/bin/
...
RUN mkdir -p /var/log/supervisor/ /etc/supervisor.d/conf.d/ /configs /etc/nginx/conf.d/ \
    ...
    && echo "upstream php-upstream { server unix:/run/php-fpm.sock; }" > /etc/nginx/conf.d/upstream.conf
...
#####################################
# Strip debug symbols from php modules, to decrease the image size:
#####################################
RUN apk add binutils \
    && find /usr/local/lib/php/extensions -iname "*.so" | xargs -I {} -n 1 strip --strip-debug {}
